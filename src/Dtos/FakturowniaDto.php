...
class FakturowniaDto
{
    ...
    public function __construct(Order $order)
    {
        ...
        $this->setBuyerEmail($order->nip ?? '');
        $this->setBuyerName($order->first_name ?? $order->user->first_name . " " . $order->last_name ?? $order->user->last_name);
        $this->setBuyerTaxNo($order->nip ?? '');
        $this->setBuyerPostCode($order->zip_code ?? '');
        $this->setBuyerCity($order->city ?? '');
        $this->setBuyerStreet($order->street ?? '');
